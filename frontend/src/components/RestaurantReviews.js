...
function RestaurantReviews({ restaurantId, itemName, onClose, onReviewAdded }) {
  ...
  const fetchReviews = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axios.get(`/restaurant-reviews/${restaurantId}`);
      setReviews(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("/restaurant-reviews", {
        restaurantId,
        ...newReview
      });
